...
hist(resi)
```
We comment that the residual histogram seems to be not normal due to small sample size of $N=12$, but the QQ-plot of the residuals aligns with the Gaussian distribution. Then lastly, we can see that a numeric test using Shapiro Wilk conveys that we don't have significant evidence to reject $H_0$ (The residuals are normally distributed). $P=0.5394  >> \alpha = 0.05 $. 
