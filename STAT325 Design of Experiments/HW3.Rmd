...
resi <- residuals(anoval_model)
shapiro.test(resi)
hist(resi)
```
We comment that the residual histogram seems very normal, and the QQ-plot of the residuals aligns with the Gaussian distribution. Then lastly, we can see that a numeric test using Shapiro Wilk conveys that we don't have significant evidence to reject $H_0$ (The residuals are normally distributed). $P=0.5394  >> \alpha = 0.05 $. 
